...
from src import preparation as pr
from src import event_prediction as ep
from src import event_timestamp_prediction as etp
...
def run(path, mode, prefix=4, epoch=1):
    ...
    if(mode == "event_prediction"):
        ...
        rnng_validation = torch.load(obj.path + "/rnnG(validation).m", weights_only=False)
        for i in range(5):
...
if __name__ == "__main__":
    dataset_path = "data/helpdesk_clean.csv" 
    run(path=dataset_path, mode="event_prediction", prefix=2, epoch=100)
